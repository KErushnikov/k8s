...
- `rocky1.k.erushnikov.ru`
- `rocky2.k.erushnikov.ru`
...
На машине `rocky2.k.erushnikov.ru` следует установить сервер OpenLDAP, так же, как и в предыдущем видео. Единственно, что не надо делать - добавлять информацию в дерево LDAP. Оно в дальнейшем автоматически будет скопировано с первого мастера.
...
В качестве slave сервера будем использовать OpenLDAP на машине `rocky2.k.erushnikov.ru`.
...
На сервере `rocky1.k.erushnikov.ru`:
...
На сервере `rocky2.k.erushnikov.ru`:
...
На slave сервере `rocky2.k.erushnikov.ru` добавим конфигурацию модуля syncprov.
...
add: olcSyncRepl
olcSyncRepl: rid=001
  provider=ldap://rocky1.k.erushnikov.ru:389/
  bindmethod=simple
...
В нашем случае достаточно на сервере `rocky1.k.erushnikov.ru` настроить репликацию данных с сервера `rocky2.k.erushnikov.ru`.
