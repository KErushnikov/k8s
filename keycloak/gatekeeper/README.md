...
Создаём клиента
    ...
    Clients -> Create
        ...
        Root URL -> https://application.k.erushnikov.ru
        Save
...
Проверяем.
    ...
    -d "username=user1" \
    -d "password=password" \
    http://keycloak.k.erushnikov.ru/iam/auth/realms/app/protocol/openid-connect/token | jq
...
Посмотрим содержимое токена.
    ...
    http://keycloak.k.erushnikov.ru/iam/auth/realms/app/protocol/openid-connect/token/introspect
...
Например:
    ...
    http://keycloak.k.erushnikov.ru/iam/auth/realms/app/protocol/openid-connect/token/introspect | jq 
